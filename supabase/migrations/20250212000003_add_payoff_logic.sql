-- Add special handling for payoff transactions
-- Payoff should: reduce current_balance, set target_amount to 0, and return excess to budget available
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    IF NEW.from_envelope_id IS NOT NULL THEN
      ...
      IF NEW.transaction_type = 'payoff' THEN
        SELECT e.envelope_type, e.current_balance INTO envelope_type, excess_amount
        FROM public.envelopes e 
        WHERE e.id = NEW.from_envelope_id;
        
        -- Calculate excess (what was allocated but not needed for payoff)
        excess_amount := excess_amount - NEW.amount;
        
        -- Update envelope: reduce current_balance by full amount, clear target_amount
