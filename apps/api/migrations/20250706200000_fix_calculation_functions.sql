-- Migration: 019_fix_calculation_functions.sql
-- Purpose: Fix ambiguous column references and type casting issues in calculation functions
-- Date: 2025-07-06

-- Fix get_budget_summary function - remove table alias conflict
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE((
            ...
            FROM public.transactions t
            WHERE t.budget_id = p_budget_id 
            AND t.transaction_type = 'allocation' 
            AND t.is_deleted = false
...
-- Fix get_payee_spending_summary function - disambiguate column references
...
-- Fix get_category_spending_breakdown function - disambiguate column references
...
-- Fix calculate_envelope_health_score function - fix type casting
...
BEGIN
    ...
    SELECT 
        EXTRACT(DAY FROM (CURRENT_DATE - MAX(t.transaction_date)))::INTEGER
